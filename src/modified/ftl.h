...
struct ftl_stats {
    uint64_t host_write_ops;
    uint64_t host_write_secs;
    uint64_t host_write_pages;

    uint64_t host_read_ops;
    uint64_t host_flush_ops;
    uint64_t host_dsm_ops;

    uint64_t host_prog_pages;
    uint64_t gc_prog_pages;
    uint64_t nand_prog_pages;

    uint64_t gc_calls;
    uint64_t gc_success;
    uint64_t gc_no_victim;

    uint64_t nand_erase_blocks;

    uint64_t hot_host_prog_pages;
    uint64_t cold_host_prog_pages;
    uint64_t hot_gc_prog_pages;
    uint64_t cold_gc_prog_pages;

    uint64_t last_dump_host_write_ops;
    uint64_t last_idle_dump_host_write_ops;
    uint64_t dump_seq;
    uint64_t last_io_time_ns;

    FILE *csv_fp;
    bool csv_open_failed;
};
...
struct ssd {
    ...
    // LPN 별로 몇 번 접근했는지 count 하기 위한 
    uint32_t *lpn_access_cnt;

    /* WAF / GC runtime statistics */
